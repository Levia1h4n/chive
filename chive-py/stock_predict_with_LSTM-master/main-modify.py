...
class Config:
    # 数据参数
    feature_columns = list(range(2, 11))     # 要作为feature的列，按原数据从0开始计算，也可以用list 如 [2,4,6,8] 设置
...
def draw(config: Config, origin_data: Data, logger, predict_norm_data: np.ndarray):
    ...
    if sys.platform.startswith('linux'):    # 无桌面的Linux下无法输出，如果是有桌面的Linux，如Ubuntu，可去掉这一行
